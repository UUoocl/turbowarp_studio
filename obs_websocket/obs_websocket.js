 ...
 * 
 * This extension is unsandboxed and uses the obs-websocket-js library (inlined).
 */
...
(function(Scratch) {
  ...
  if (!Scratch.extensions.unsandboxed) {
    throw new Error('OBS WebSocket extension must run unsandboxed');
  }
  ...
  class OBSWebSocketExtension {
    constructor() {
      ...
      // Default subscriptions (Everything except high-volume events)
      this.subscriptions = {
        General: true,
        Config: true,
        Scenes: true,
      ...
      this.sceneCache = []; // Array of {sceneName, items: [{name, id}]}
    ...
    getInfo() {
      return {
        ...
        blocks: [
          ...
          {
             opcode: 'getEventProperty',
             blockType: Scratch.BlockType.REPORTER,
             text: 'event data [PROP]',
             arguments: {
               PROP: { type: Scratch.ArgumentType.STRING, defaultValue: 'sceneName' }
             }
          }
        ],
        menus: {
          ...
          SUBS_MENU: {
            acceptReporters: true,
            items: Object.keys(this.subscriptions)
          },
          STATE_MENU: {
            acceptReporters: true,
            items: ['true', 'false']
    ...
    getItemMenu() {
      ...
      this.sceneCache.forEach(scene => {
        scene.items.forEach(item => {
          // We return an object so TurboWarp shows the text but gives us the value
          items.push({
    ...
    async refreshCache() {
      if (!this.obs || !this.connected) {
         console.warn("OBS not connected, cannot refresh cache");
         return;
      }
      try {
        ...
        for (const scene of sceneList.scenes) {
          const itemList = await this.obs.call('GetSceneItemList', { sceneName: scene.sceneName });
          newCache.push({
            sceneName: scene.sceneName,
            items: itemList.sceneItems.map(i => ({
              name: i.sourceName,
              id: i.sceneItemId
            }))
          });
        }
        this.sceneCache = newCache;
        console.log("OBS Scene/Item cache updated:", this.sceneCache);
      } catch (err) {
        console.error("Failed to refresh OBS cache:", err);
      }
    }

    async setSceneItemEnabled(args) {
      if (!this.obs || !this.connected) return;
      
      let sceneName = args.SCENE;
      let itemId = -1;
      
      // Check if ITEM is from our menu (contains |)
      if (args.ITEM.includes('|')) {
        const parts = args.ITEM.split('|');
        sceneName = parts[0];
        itemId = parseInt(parts[1]);
      } else {
        // User typed the item name or used a reporter
        // Try to find it in the selected scene
        const scene = this.sceneCache.find(s => s.sceneName === sceneName);
        if (scene) {
          const item = scene.items.find(i => i.name === args.ITEM);
          if (item) itemId = item.id;
        }
      }

      if (itemId === -1) {
        console.warn(`Could not find item ID for "${args.ITEM}" in scene "${sceneName}"`);
        return;
      }
      ...
      try {
        ...
        this.obs.emit = (event, data) => {
          ...
          if (!internalEvents.includes(event)) {
            ...
            // Generic Event Hat
            // We use the lowercase ID 'obswebsocket'
            Scratch.vm.runtime.startHats('obswebsocket_whenEvent', {
              EVENT: event
            });
            
            // Dedicated Event Hats
            if (event === 'CurrentProgramSceneChanged') Scratch.vm.runtime.startHats('obswebsocket_whenSceneChanged');
            if (event === 'RecordStateChanged') Scratch.vm.runtime.startHats('obswebsocket_whenRecordStateChanged');
            if (event === 'StreamStateChanged') Scratch.vm.runtime.startHats('obswebsocket_whenStreamStateChanged');
            if (event === 'VirtualcamStateChanged') Scratch.vm.runtime.startHats('obswebsocket_whenVirtualCamStateChanged');
          }
        ...
        // Calculate bitmask
        let subs = 0;
        const SubEnum = OBSWebSocket.EventSubscription;
        if (this.subscriptions.General) subs |= SubEnum.General;
        if (this.subscriptions.Config) subs |= SubEnum.Config;
        if (this.subscriptions.Scenes) subs |= SubEnum.Scenes;
        if (this.subscriptions.Inputs) subs |= SubEnum.Inputs;
        if (this.subscriptions.Transitions) subs |= SubEnum.Transitions;
        if (this.subscriptions.Filters) subs |= SubEnum.Filters;
        if (this.subscriptions.Outputs) subs |= SubEnum.Outputs;
        if (this.subscriptions.SceneItems) subs |= SubEnum.SceneItems;
        if (this.subscriptions.MediaInputs) subs |= SubEnum.MediaInputs;
        if (this.subscriptions.Vendors) subs |= SubEnum.Vendors;
        if (this.subscriptions.Ui) subs |= SubEnum.Ui;
        
        if (this.subscriptions.InputVolumeMeters) subs |= SubEnum.InputVolumeMeters;
        if (this.subscriptions.InputActiveStateChanged) subs |= SubEnum.InputActiveStateChanged;
        if (this.subscriptions.InputShowStateChanged) subs |= SubEnum.InputShowStateChanged;
        if (this.subscriptions.SceneItemTransformChanged) subs |= SubEnum.SceneItemTransformChanged;

        await this.obs.connect(url, password, { eventSubscriptions: subs });
    ...
    async switchScene(args) {
      if (!this.obs || !this.connected) return;
      await this.obs.call('SetCurrentProgramScene', { sceneName: args.SCENE });
    }

    async controlStream(args) {
      if (!this.obs || !this.connected) return;
      await this.obs.call(args.ACTION + 'Stream');
    }

    async controlRecord(args) {
      if (!this.obs || !this.connected) return;
      await this.obs.call(args.ACTION + 'Record');
    }

    async controlVirtualCam(args) {
      if (!this.obs || !this.connected) return;
      await this.obs.call(args.ACTION + 'VirtualCam');
    }

    async setInputMute(args) {
      if (!this.obs || !this.connected) return;
      await this.obs.call('SetInputMute', { inputName: args.INPUT, inputMuted: args.STATE === 'true' });
    }

    async toggleInputMute(args) {
      if (!this.obs || !this.connected) return;
      await this.obs.call('ToggleInputMute', { inputName: args.INPUT });
    }

    async setInputVolume(args) {
      if (!this.obs || !this.connected) return;
      // OBS uses a multiplier (0.0 to 1.0) or dB. Percent is easiest for Scratch.
      const vol = parseFloat(args.VOLUME) / 100;
      await this.obs.call('SetInputVolume', { inputName: args.INPUT, inputVolumeMul: vol });
    }

    async sendRequest(args) {
      if (!this.obs || !this.connected) return 'Not connected';
      try {
        const method = args.METHOD;
        const params = JSON.parse(args.JSON || '{}');
        const response = await this.obs.call(method, params);
    ...
    getEventProperty(args) {
      const prop = args.PROP;
      if (this.lastEventData && this.lastEventData.hasOwnProperty(prop)) {
        return this.lastEventData[prop];
      }
      return '';
    }
    ...
    whenSceneChanged() { return true; }
    whenRecordStateChanged() { return true; }
    whenStreamStateChanged() { return true; }
    whenVirtualCamStateChanged() { return true; }
  }

  Scratch.extensions.register(new OBSWebSocketExtension());
})(Scratch);
