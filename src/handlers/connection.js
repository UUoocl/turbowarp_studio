...
export const connect = async (extension, args) => {
  ...
  try {
    ...
    const originalEmit = extension.obs.emit;
    extension.obs.emit = (event, data) => {
      ...
      if (!internalEvents.includes(event)) {
        ...
        if (event === 'SceneItemTransformChanged' && data.sceneName === extension.currentSceneName) {
           const item = extension.currentSceneItems.find(i => i.sceneItemId === data.sceneItemId);
           if (item) item.sceneItemTransform = data.sceneItemTransform;
        }
        if (event === 'SceneItemEnableStateChanged' && data.sceneName === extension.currentSceneName) {
           const item = extension.currentSceneItems.find(i => i.sceneItemId === data.sceneItemId);
...
export const disconnect = async (extension) => {
  if (extension.obs) {
    try {
...
export const isConnected = (extension) => {
