...
var OBSWebSocketExtensionBundle = (() => {
  ...
  var getMenus = (extension2) => {
    return {
      ...
      SUBS_MENU: {
        acceptReporters: true,
        items: Object.keys(extension2.subscriptions)
      },
      STATE_MENU: {
        acceptReporters: true,
        items: ["true", "false"]
  ...
  var BaseOBSWebSocket = class _BaseOBSWebSocket extends import_index.default {
     ...
     */
    async disconnect() {
      if (!this.socket || this.socket.readyState === browser_default.CLOSED) {
        return;
      }
      const connectionClosedPromise = this.internalEventPromise("ConnectionClosed");
      this.socket.close();
    ...
    async createConnection(url) {
      ...
      this.socket = new browser_default(url, this.protocol);
  ...
  // node_modules/obs-websocket-js/dist/json.js
  ...
  var connect = async (extension2, args) => {
    ...
    try {
      ...
      extension2.obs.emit = (event, data) => {
        ...
        return originalEmit.call(extension2.obs, event, data);
      };
      let subs = 0;
      const SubEnum = EventSubscription || {
        General: 1,
        Config: 2,
        Scenes: 4,
        Inputs: 8,
      ...
      Object.keys(extension2.subscriptions).forEach((key) => {
        if (extension2.subscriptions[key] && SubEnum[key]) {
          subs |= SubEnum[key];
        }
      });
      await extension2.obs.connect(url, password, { eventSubscriptions: subs });
      extension2.connected = true;
      console.log("OBS Connected with subs:", subs);
      await extension2.updateVideoSettings();
      await extension2.updateCurrentSceneItems();
    } catch (err) {
      console.error("OBS Connection Error:", err);
      extension2.connected = false;
      extension2.obs = null;
    }
  };
  var disconnect = async (extension2) => {
    if (extension2.obs) {
      try {
        await extension2.obs.disconnect();
      } catch (e) {
      }
      extension2.obs = null;
    }
  ...
  var scratchScale = (val, isWidth, videoSettings, stageWidth = 480, stageHeight = 360) => {
    if (isWidth) {
  ...
  var refreshCache = async (extension2) => {
    ...
    try {
      ...
      console.log("OBS Cache Updated:", extension2.sceneCache.length, "scenes");
      await extension2.updateCurrentSceneItems();
    } catch (e) {
      console.error("Failed to refresh OBS cache:", e);
    }
  };
  var setSceneItemEnabled = async (extension2, args) => {
    if (!extension2.obs || !extension2.connected)
      return;
    ...
    await extension2.obs.call("SetSceneItemEnabled", {
      sceneName,
      sceneItemId: itemId,
      sceneItemEnabled: args.STATE === "true"
    });
  };
  var switchScene = async (extension2, args) => {
    if (!extension2.obs || !extension2.connected)
      return;
    await extension2.obs.call("SetCurrentProgramScene", { sceneName: args.SCENE });
  };
  var getCurrentSceneName = (extension2) => {
    return extension2.currentSceneName;
  };
  var getSourceCount = (extension2) => {
    return extension2.currentSceneItems.length;
  };
  var getSourceData = (extension2, args) => {
    const index = Math.round(args.INDEX) - 1;
    if (index < 0 || index >= extension2.currentSceneItems.length)
      return "";
    const item = extension2.currentSceneItems[index];
    const prop = args.PROP;
    if (prop === "name")
      return item.sourceName;
    if (prop === "type")
      return item.inputKind || item.sourceType || "unknown";
    if (prop === "enabled")
      return item.sceneItemEnabled ? "true" : "false";
    const transform = item.sceneItemTransform;
    const stageWidth = extension2.runtime.stageWidth;
    const stageHeight = extension2.runtime.stageHeight;
  ...
  var getSourceTransform = (extension2, args) => {
    const name = args.NAME;
    const item = extension2.currentSceneItems.find((i) => i.sourceName === name);
    if (!item)
      return "";
    const prop = args.PROP;
    if (prop === "enabled")
  ...
  var controlStream = async (extension2, args) => {
    if (!extension2.obs || !extension2.connected)
      return;
    await extension2.obs.call(args.ACTION + "Stream");
  };
  var controlRecord = async (extension2, args) => {
    if (!extension2.obs || !extension2.connected)
      return;
    await extension2.obs.call(args.ACTION + "Record");
  };
  var controlVirtualCam = async (extension2, args) => {
    if (!extension2.obs || !extension2.connected)
      return;
  ...
  var toggleInputMute = async (extension2, args) => {
    if (!extension2.obs || !extension2.connected)
      return;
    await extension2.obs.call("ToggleInputMute", { inputName: args.INPUT });
  };
  var setInputVolume = async (extension2, args) => {
    if (!extension2.obs || !extension2.connected)
      return;
  ...
  var whenRecordStateChanged = () => true;
  ...
  var sendRequest = async (extension2, args) => {
    ...
    try {
      ...
      const response = await extension2.obs.call(method, params);
  ...
  var getLastEventName = (extension2) => {
    return extension2.lastEventType;
  };
  var getLastEventData = (extension2) => {
    return JSON.stringify(extension2.lastEventData);
  };
  ...
  var OBSWebSocketExtension = class {
    ...
    getSceneMenu() {
      if (this.sceneCache.length === 0)
        return ['Click "Update Lists" first'];
    ...
    refreshCache() {
      return refreshCache(this);
    }
    setSceneItemEnabled(args) {
      return setSceneItemEnabled(this, args);
    }
    switchScene(args) {
      return switchScene(this, args);
    }
    getCurrentSceneName() {
      return getCurrentSceneName(this);
    }
    getSourceCount() {
      return getSourceCount(this);
    }
    getSourceData(args) {
      return getSourceData(this, args);
    }
    getSourceTransform(args) {
      return getSourceTransform(this, args);
    }
    getAllSourcesJSON() {
      return getAllSourcesJSON(this);
    }
    controlStream(args) {
      return controlStream(this, args);
    }
    controlRecord(args) {
      return controlRecord(this, args);
    }
    controlVirtualCam(args) {
      return controlVirtualCam(this, args);
    }
    setInputMute(args) {
      return setInputMute(this, args);
    }
    toggleInputMute(args) {
      return toggleInputMute(this, args);
    }
    setInputVolume(args) {
      return setInputVolume(this, args);
    }
    setSpriteStretch(args, util) {
      return setSpriteStretch(this, args, util);
    }
    whenSceneBecomes(args) {
      return whenSceneBecomes(this, args);
    }
    whenEvent(args) {
      return whenEvent(this, args);
    }
    whenSceneChanged() {
      return whenSceneChanged();
    }
    whenRecordStateChanged() {
      return whenRecordStateChanged();
    }
    whenStreamStateChanged() {
      return whenStreamStateChanged();
    }
    whenVirtualCamStateChanged() {
      return whenVirtualCamStateChanged();
    }
    sendRequest(args) {
      return sendRequest(this, args);
    }
    setSubscription(args) {
      return setSubscription(this, args);
    }
    getLastEventName() {
      return getLastEventName(this);
    }
    getLastEventData() {
      return getLastEventData(this);
    }
    getEventProperty(args) {
      return getEventProperty(this, args);
    }
  };
